...
describe('ManagerReportsService', () => {
  ...
  describe('generate', () => {
    it('should throw UnprocessableEntityException if mandatory items are incomplete', async () => {
      ...
      lineItemRepo
        .createQueryBuilder()
        .getMany.mockResolvedValue([{ id: 'li-1' }]);

      await expect(service.generate('audit-1', mockUser)).rejects.toThrow(
        UnprocessableEntityException,
      );
    });
    ...
    it('should trigger generation if all items are complete', async () => {
      ...
      expect(aiJobsService.send).toHaveBeenCalledWith(
        'report-generation',
        expect.anything(),
      );
    });
  ...
  describe('finalize', () => {
    it('should mark audit as closed', async () => {
      const mockReport = {
        id: 'r-1',
        status: ReportStatus.SENT_FOR_CLIENT_REVIEW,
        audit: { id: 'a-1', managerId: 'm-1', clientId: 'c-1' },
      };
