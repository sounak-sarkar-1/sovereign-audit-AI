  ...
  async getHeatmap(managerId: string, auditId?: string) {
    ...
    const heatmapData = await Promise.all(auditors.map(async (auditor) => {
      ...
      const auditDetails = await Promise.all([...new Set(auditorAssignments.map(a => a.auditId))].map(async (aId) => {
        ...
        const openItems = lineItems.filter(li => li.status !== LineItemStatus.SUBMITTED && li.status !== LineItemStatus.EXCEPTION_APPROVED).length;
    ...
    const totalItems = heatmapData.reduce((sum, a) => sum + (a.openLineItems / (a.completionPercent / 100 || 1) || 0), 0); // rough estimate
