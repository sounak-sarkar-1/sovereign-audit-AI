...
export class ReportGenerationWorker implements OnModuleInit {
  ...
  async onModuleInit() {
    await this.aiJobsService.work('report-generation', (job) =>
      this.handleJob(job),
    );
  ...
  private async handleJob(job: any) {
    ...
    try {
              ...
              ...exceptions.map(
                (ex) =>
                  new Paragraph({
                    children: [
                      new TextRun({
                        text: `[EXCEPTION] ${ex.auditScopeLineItem?.name}: `,
                        bold: true,
                        color: 'FF0000',
                      }),
                  ...
                  ...lineItems.map(
                    (li) =>
                      new TableRow({
                        children: [
                          new TableCell({
                            children: [new Paragraph({ text: li.name })],
                          }),
                          new TableCell({
                            children: [new Paragraph({ text: li.status })],
                          }),
                        ],
                      }),
                  ),
      ...
      // 4. Update Database
      const reportToUpdate = await this.reportRepo.findOne({
        where: { id: reportId },
      });
      if (reportToUpdate) {
